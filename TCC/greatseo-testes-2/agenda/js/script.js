       ...
       /* function removeEvent(id){
        ...
        $(document).on('click', '.showEvent', function(){
           ...
           $('.events-today').html("");
            ...
            $.ajax({
              type : "POST",
              url : "http://localhost/web-app/tcc/greatseo-testes-2/ajax/consulta_agenda.php", 
              data: { date: date,  },
              success : function(response) {
               ...
               console.log(alunos); 
                ...
                var content = `<div class="alert alert-dark alert-dismissible fade show" id="hr_alunos" role="alert" style="font-size: 13px;">`;
                content +=  `<i class="fas fa-user-clock"></i>&nbsp;`;
                content +=  alunos[i].nome;
                content +=  " - ";
                content +=  alunos[i].horaInicial;
                content +=  " / ";
                content +=  alunos[i].horaFinal;
                content +=  `&nbsp;&nbsp; <i class="fas fa-car"></i>`;
                content +=  alunos[i].veiculo;
                content +=  `&nbsp;&nbsp; <i class="fas fa-user"></i>`;
                content +=  alunos[i].instrutor;
                content +=  "&nbsp;&nbsp;&nbsp;&nbsp;";

                content +=`<button type="button" id="update_aula" class="update btn btn-outline-secondary" data-idAula="${alunos[i].cod_age}"  onclick="getCodAula(this)">
                      <i class="fas fa-edit"></i>
                    </button>`;
                content +=`<button type="button" class="close remove-event" data-idAula="${alunos[i].cod_age}" data-dismiss="alert" aria-label="Close" onclick="getCodAulaDel(this);delete_aluno()">
